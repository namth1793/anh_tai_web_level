...
router.post('/register', (req, res) => {
  const { name, email, password, student_id, major } = req.body;
  ...
  try {
    ...
    const result = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `).run(name, email, hashed, student_id || null, major || '');
...
router.put('/profile', auth, (req, res) => {
  const { name, student_id, major, avatar } = req.body;
  db.prepare(`UPDATE users SET name = ?, student_id = ?, major = ?, avatar = ? WHERE id = ?`)
    .run(name || req.user.name, student_id || req.user.student_id, major || req.user.major, avatar || req.user.avatar, req.user.id);
